...
// *case. 3 현재 자신이 춥다(true), 안춥다(false)를 cold 변수에 담아 자료형으로 출력.
function check1(){
    var cold = false;
    console.log('cold 변수에 현재 담긴 값인 ' + cold + '의 자료 유형은 ' + typeof(cold));
}
// *case. 4 test변수를 선언만 하고 초기화 하지 않은 상태에서 자료형을 check해보자.
function check2(){
    var test;
    console.log('초기화 하지 않은 변수의 자료유형은' + typeof(test) + '입니다.');
}
// *case. 5 obj 변수에 객체를 담을 예정이니 null로 초기화해서 자료형을 check해보자.
function check3(){
    var obj = null;
